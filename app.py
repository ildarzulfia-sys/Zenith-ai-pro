...
if st.button(f"🚀 Просканировать {sport_choice}"):
    # Используем формат URL, подходящий для твоей подписки
    ...
    with st.spinner(f'Запрашиваю данные из глобальной базы на {selected_date}...'):
        try:
                    ...
                    with st.expander(f"🏟 {league_name} ({country})"):
                        for event in stage.get('Events', []):
                            ...
                            if score_h is not None:
                                # Матч завершен или идет сейчас
                                st.write(f"🕒 **{status}** | **{home}** {score_h} : {score_a} **{away}**")
                            else:
                                # Матч еще не начался
                                raw_time = str(event.get('Esd', '00000000000000'))
                                start_t = f"{raw_time[8:10]}:{raw_time[10:12]}" if len(raw_time) >= 12 else "Скоро"
                                st.write(f"📅 {start_t} | {home} vs {away}")
            else:
                st.info(f"На {selected_date} матчей по {sport_choice} не найдено.")
                # Если сервер вернул ошибку, выводим её ниже
                if 'message' in data:
